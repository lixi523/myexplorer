...
/// Hard cap on how long a synchronous `myexplorer.exec` may run. A blocked child
...
/// wedging the plugin isolate. Slow work belongs in `myexplorer.run_task`.
const EXEC_TIMEOUT: Duration = Duration::from_secs(5);

/// Cap on the size of a file `myexplorer.read_text` will load, in bytes.
...
fn install_api(lua: &Lua, effects: &Effects) -> mlua::Result<Table> {
    let myexplorer = lua.create_table()?;
...
fn load_impl(path: &str) -> mlua::Result<Json> {
    ...
    lua.globals().set("myexplorer", myexplorer)?;
...
fn bar_update_impl(path: &str, bar_id: &str, ctx_json: &str) -> mlua::Result<Json> {
    ...
    let effects: Effects = Rc::new(RefCell::new(Vec::new()));
    let myexplorer = install_api(&lua, &effects)?;
    myexplorer.set("register", lua.create_function(move |_, _: Table| Ok(()))?)?;
...
#[no_mangle]
pub unsafe extern "C" fn myexplorer_plugin_load(path: *const c_char) -> *mut c_char {
    if path.is_null() {
...
#[no_mangle]
pub unsafe extern "C" fn myexplorer_plugin_invoke(
    path: *const c_char,
...
#[no_mangle]
pub unsafe extern "C" fn myexplorer_plugin_bar_update(
    path: *const c_char,
    bar_id: *const c_char,
    ctx_json: *const c_char,
) -> *mut c_char {
...
#[no_mangle]
pub unsafe extern "C" fn myexplorer_plugin_bar_click(
    path: *const c_char,
    bar_id: *const c_char,
...
/// All pointer args must be valid NUL-terminated UTF-8 C strings.
#[no_mangle]
pub unsafe extern "C" fn myexplorer_plugin_column_compute(
    path: *const c_char,
...
#[no_mangle]
pub unsafe extern "C" fn myexplorer_plugin_str_free(ptr: *mut c_char) {
    if !ptr.is_null() {
