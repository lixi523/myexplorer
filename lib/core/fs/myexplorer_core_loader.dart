...
/// Thrown when the required `myexplorer_core` native library cannot be loaded.
/// `myexplorer_core` is a hard dependency: directory listing, recursive search
...
class MyExplorerCoreException implements Exception {
  final String message;
  const MyExplorerCoreException(this.message);
  @override
  String toString() => 'MyExplorerCoreException: $message';
}
...
class MyExplorerTrashFailure {
  final String path;
  final String message;

  const MyExplorerTrashFailure(this.path, this.message);
}
...
/// Loads the required `myexplorer_core` native helper (Rust). Cached.
/// [requireLib] throws [MyExplorerCoreException] if it is absent so callers
/// fail loudly instead of silently degrading.
class MyExplorerCoreLoader {
  MyExplorerCoreLoader._();
  ...
  static DynamicLibrary? load() {
    ...
    for (final path in _candidatePaths()) {
      try {
        ...
        final abi = lib.lookupFunction<_AbiNative, _AbiDart>(
          'myexplorer_core_abi',
        );
        final value = abi();
  ...
  /// Throws [MyExplorerCoreException] if the library is missing.
  static Uint8List? search(
    ...
    final free = lib.lookupFunction<_FreeNative, _FreeDart>('myexplorer_free');
  ...
  static Uint8List? listDir(String path, {bool withStat = true}) {
    final lib = requireLib();
    final list = lib.lookupFunction<_ListNative, _ListDart>('myexplorer_list');
    final free = lib.lookupFunction<_FreeNative, _FreeDart>('myexplorer_free');
  ...
  static Uint8List? enumerate(String root, {bool postorder = true}) {
    final lib = requireLib();
    final fn = lib.lookupFunction<_EnumNative, _EnumDart>(
      'myexplorer_enumerate',
    );
    final free = lib.lookupFunction<_FreeNative, _FreeDart>('myexplorer_free');
    final rootPtr = root.toNativeUtf8();
    final outLen = calloc<IntPtr>();
  ...
  static List<MyExplorerTrashFailure> trash(List<String> paths) {
    if (paths.isEmpty) return const [];
    final lib = requireLib();
    final fn = lib.lookupFunction<_TrashNative, _TrashDart>('myexplorer_trash');
    final free = lib.lookupFunction<_FreeNative, _FreeDart>('myexplorer_free');
  ...
  static List<MyExplorerTrashFailure> trashRestore(List<String> ids) =>
      _trashById('myexplorer_trash_restore', ids);

  static List<MyExplorerTrashFailure> trashPurge(List<String> ids) =>
      _trashById('myexplorer_trash_purge', ids);

  static List<MyExplorerTrashFailure> _trashById(
    String symbol,
    List<String> ids,
  ) {
  ...
  static String? buildInfo() {
    ...
    int abi() {
      try {
        return lib.lookupFunction<_AbiNative, _AbiDart>(
          'myexplorer_core_abi',
        )();
      } catch (e, st) {
    ...
    return '${sym('myexplorer_core_version')} (${sym('myexplorer_core_git')}) '
        'abi=${abi()}';
  }
  ...
  static Uint8List? sftpRead(
    ...
    final fn = rangeFn == null
        ? lib.lookupFunction<_SftpReadNative, _SftpReadDart>(
            'myexplorer_sftp_read',
          )
        : null;
    final free = lib.lookupFunction<_FreeNative, _FreeDart>('myexplorer_free');
    final pathP = path.toNativeUtf8();
  ...
  static bool supportsPty() {
    ...
    try {
      lib.lookupFunction<_PtyOpenNative, _PtyOpenDart>('myexplorer_pty_open');
  ...
  static void ptyWrite(int id, Uint8List data) {
    ...
    final fn = _ptyWriteFn ??= lib
        .lookupFunction<_PtyWriteNative, _PtyWriteDart>('myexplorer_pty_write');
    final dataPtr = calloc<Uint8>(data.length);
    try {
      dataPtr.asTypedList(data.length).setRange(0, data.length, data);
  ...
  static bool ptyAlive(int id) {
    ...
    final fn = _ptyAliveFn ??= lib
        .lookupFunction<_PtyAliveNative, _PtyAliveDart>('myexplorer_pty_alive');
  ...
  /// `myexplorer_core`. Empty string when the core was found by bare name, letting
  ...
  static List<String> _candidatePaths() {
    ...
    return [devTarget, p.join(exeDir, 'lib', lib), p.join(exeDir, lib), lib];
  }

  static List<MyExplorerTrashFailure> _decodeTrashFailures(Uint8List bytes) {
    final failures = <MyExplorerTrashFailure>[];
    final data = ByteData.sublistView(bytes);
    var off = 0;
    while (off + 8 <= bytes.length) {
      ...
      off += msgLen;
      failures.add(MyExplorerTrashFailure(path, msg));
    }
